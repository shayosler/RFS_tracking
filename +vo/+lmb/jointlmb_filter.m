function [tt_lmb_update_k, X, N, L] = jointlmb_filter(tt_lmb_update, model, filter, meas, k)
    
    %joint predict and update, results in GLMB, convert to LMB
    glmb_update= jointlmbpredictupdate(tt_lmb_update,model,filter,meas,k);              
    T_predict= length(tt_lmb_update)+model.T_birth;
    tt_lmb_update= glmb2lmb(glmb_update);                                               
    T_posterior= length(tt_lmb_update);
       
    %pruning, truncation and track cleanup
    tt_lmb_update_k= clean_lmb(tt_lmb_update,filter);                                     
    T_clean= length(tt_lmb_update_k);
    
    %state estimation
    [X, N, L]= extract_estimates(tt_lmb_update_k,model);
...
if filter.gate_flag
    for tabidx=1:length(tt_predict)
        tt_predict{tabidx}.gatemeas= gate_meas_gms_idx(meas.Z,filter.gamma,model,tt_predict{tabidx}.m,tt_predict{tabidx}.P);
    end
else
    for tabidx=1:length(tt_predict)
        tt_predict{tabidx}.gatemeas= 1:size(meas.Z,2);
    end
end
...
m= size(meas.Z,2);                                   %number of measurements
...
for tabidx= 1:length(tt_predict)
    for emm= tt_predict{tabidx}.gatemeas
            ...
            [qz_temp,m_temp,P_temp] = kalman_update_multiple(meas.Z(:,emm),model,tt_predict{tabidx}.m,tt_predict{tabidx}.P);             %kalman update for this track and this measurement
