function [tt_lmb_update_k, X, N, L] = lmb_filter(tt_lmb_update, model, filter, meas, k)
    %prediction
    ...
    glmb_update= update(glmb_predict,model,filter,meas);
    ...
    %display diagnostics
    %display_diaginfo(tt_lmb_update_k,k,est,filter,T_predict,T_posterior,T_clean);
end
...
function glmb_update= update(glmb_predict,model,filter,meas)
%gating by tracks
if filter.gate_flag
    m_tracks= [];
    P_tracks= [];
    ...
    meas.Z= gate_meas_gms(meas.Z,filter.gamma,model,m_tracks,P_tracks);
end
...
for emm= 1:m
    for tabidx= 1:length(glmb_predict.tt)
        ...
        [qz_temp,m_temp,P_temp] = kalman_update_multiple(meas.Z(:,emm),model,glmb_predict.tt{tabidx}.m,glmb_predict.tt{tabidx}.P);
